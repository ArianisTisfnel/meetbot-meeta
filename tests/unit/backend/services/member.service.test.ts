...
describe('updateMemberPermissions', () => {
  ...
  it('throws 403 PERMISSION_DENIED when caller is not the project owner', async () => {
    mockPrisma.project.findUnique.mockResolvedValueOnce({
      ...MOCK_PROJECT,
      ownerUserId: 99, // different owner
    })
  ...
  it('強制矯正 canView=false 回 true（檢視為基準權限，不可取消）', async () => {
    ...
    mockPrisma.projectMember.findUnique.mockResolvedValueOnce({ id: 'mem-1', userId: 2 })
    ...
    mockPrisma.user.findUnique.mockResolvedValueOnce({ email: 'm@x.com' })
...
describe('removeMember', () => {
  ...
  it('throws 403 PERMISSION_DENIED when trying to remove the project owner', async () => {
    mockPrisma.project.findUnique.mockResolvedValueOnce(MOCK_PROJECT) // ownerUserId = 1

    await expect(
      removeMember('proj-1', 1, 1), // targetUserId === ownerUserId
