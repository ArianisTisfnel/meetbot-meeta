...
describe('isStopCommand — 共用判準', () => {
  ...
  // 實測 2026-08-26（多人會議專屬——只有多人才會坐在會議裡討論這個功能本身）：
  // stripWakeWord 無錨定，會把喚醒詞之前的整段吃掉，於是句中的「蜜塔閉嘴」剝完
  // 變成 ^閉嘴… → 判成叫停 → 她該回答時反而裝死。
  it('句中提到「蜜塔閉嘴」的討論不算叫停（喚醒詞不在呼喚位置就不剝）', () => {
    for (const t of [
      '就是像我們叫蜜塔閉嘴或者是',
      '你們剛剛講到那個 就是說教蜜塔閉嘴的那個東西',
      '我在想要不要讓蜜塔安靜一點比較好',
    ]) {
      expect(isStopCommand(t)).toBe(false)
    }
  ...
  it('喚醒詞在呼喚位置（含前置贅詞）仍然認得出叫停', () => {
    for (const t of ['蜜塔閉嘴', '那個蜜塔閉嘴', '欸蜜塔安靜']) {
      expect(isStopCommand(t)).toBe(true)
    }
  })

  // ── 以下全部取自真實會議 log（2026-07-29 ~ 08-29 的全檔掃描）─────────────
  // stop-only 模式下叫停是**唯一**的打斷路徑，漏一種講法就等於叫不停她。

  it('禮貌／迂迴的叫停也算數（原本全部漏掉）', () => {
    for (const t of [
      '蜜塔,你可以閉嘴嗎?', // 08-17 20:52 WENDY
      '你可以安靜囉。', // 08-16 23:57 WENDY
      '蜜塔請安靜', // 08-16 23:59 WENDY
      '你閉嘴就好了', // 07-29 22:56 WENDY
      '蜜塔可以先不用來補充一下', // 08-26 10:24 Arianis
      '蜜塔你可以先停一下',
      '你先不要講了好嗎',
    ]) {
      expect(isStopCommand(t)).toBe(true)
  ...
  it('英文叫停也算數', () => {
    for (const t of ['Quiet.', 'Stop', 'shut up', 'Silence']) {
      expect(isStopCommand(t)).toBe(true)
    }
  })

  it('STT 誤轉的喚醒詞（畢達／碧塔／嘿塔／密卡／Vita）仍剝得掉', () => {
    for (const t of ['畢達,你可以安靜了', '碧塔閉嘴', '嘿塔安靜', '密卡閉嘴', 'Vita安靜']) {
      expect(isStopCommand(t)).toBe(true)
    }
  })

  // 這是全 log 單一時刻最大的誤判爆量：同一秒 82 筆 partial 全被當成叫停。
  // 分界不是長度，是「閉嘴」後面接不接「的」——接了就是名詞化，在談論這個動作。
  it('叫停詞後接「的」＝在討論，不是命令', () => {
    for (const t of [
      '密塔閉嘴的話,他可能就是多人的時候收音的效果會不是很好', // 08-26 10:23:23
      '就是說教蜜塔閉嘴的那個東西',
    ]) {
      expect(isStopCommand(t)).toBe(false)
    }
  ...
  it('問她功能、談論她，都不算叫停（放寬詞表後的護欄）', () => {
    for (const t of [
      '蜜塔,我可以叫你閉嘴嗎?', // 08-14 15:50 —— 主語是「我」，在問功能
      '蜜塔,為什麼我要說兩次閉嘴你才聽得到啊?', // 08-17 18:53
      '可是其實插嘴,就是叫他閉嘴,這個不是必要的對不對?', // 08-23 22:00
      '在場會議裡面的人都不能講閉嘴啊', // 08-23 22:01
      '照理來講,就是應該是它的閉嘴功能會比較穩', // 08-26 10:15
      '蜜塔,停車場在哪裡',
      '這個功能停在哪一版',
    ]) {
      expect(isStopCommand(t)).toBe(false)
