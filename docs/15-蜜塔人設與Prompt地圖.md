...
| **每輪語意決策（定址／問題／意圖／插話，四合一）** | `backend/src/sessions/interjection-prompts.ts` `TURN_DECISION_SYSTEM`（邏輯在 `response-policy.ts` `decideTurn`） | **兩個都要跑**：`scripts/eval-meeting.ts --address`（定址、連續追問）＋`scripts/eval-interjection.ts --variant live`（插話，26 劇本，見第六節） |
...
- **2026-08-18 真會議（第 14 次，指導老師在場）暴露的三個問題，已全數修在 prompt 層**。
  這一輪的價值不在改了什麼，而在**為什麼離線評測全綠卻還是炸了**——見最下方的環境落差那條。
  - ① **追問窗在「整場都在談蜜塔」的會議等於全程開著**：引擎側 `FOLLOWUP_WINDOW_MS`（90 秒）
    只是成本閘門，一場一直在叫她的會議永遠在窗內，於是每一句都送語意層裁決。
    實際誤答的是 STT 切出來的半句（「這個是」「等一下」）與對全場的招呼（「大家好」），
    以及**照唸規格文字**時引號裡的喚醒詞（「與會者會喊『蜜塔』來提問」，連續誤觸三次）。
    → `TURN_DECISION_SYSTEM` ① 段新增四條「整場都在談蜜塔」專用判準，
    並把追問串的關閉條件寫死成三個同時成立（同一人／同一話題／上一則確實是蜜塔在回答）。
  - ② **有人在連續報告時觸發冷場插話**：引擎用「逐字稿多久沒有新內容」推估對方講完了，
    轉錄一延遲就把「還在講」讀成冷場。**不改 `interjection.ts` 的計時**（那不是 prompt 分工），
    改成在 ④ 段要求模型自己擋：同一人連續佔多則、最後一則是報告內容的一部分、
    或以連接詞／助詞收尾（然後、就是、所以、那個）→ 一律 false。
  - ③ **對指導老師編造本系統的實作細節**（「自動補位邏輯過於激進」「VAD 門檻」「事件驅動流程圖」
    全是本專案沒有的東西）。**兇手不是 Dify**——那幾則的句尾都是「目前還缺…」，
    那是 `wake-word-detector.ts` `answerFromTranscript` system prompt 的最後一行；
    同一份 prompt 第 2 點又寫「意見型…不要推託說無法回答」，等於明文授權硬掰。
    → 在該處開一個**只圈住「蜜塔自身實作」**的禁區（凌駕第 2 點），意見型本來的能力不動；
    `resolveAnswerRouted` 的 hybrid 合成與 `answerChitchat` 同步補上同一道禁區
    （「你怎麼一直插話」這類問自身行為的話會分到 chitchat，見 ③ 段最後一條）。
  - **定址評測基準（2026-08-18，19 劇本 54 案例）：54/54，回歸失敗 0。**
    跑法：`scripts/eval-meeting.ts --address`。
  - 收尾時踩到的兩件事，都寫下來免得下次重演：
    - **「上一則必須是蜜塔在回答」這種條件在評測裡永遠不成立**——`eval-meeting` 的對話窗
      不含蜜塔的回覆（第六節前面已記過這個落差），我第一版把它寫進追問串的關閉條件，
      直接讓真追問整批漏答。**寫判準前先確認那個訊號在輸入裡到底存不存在。**
    - **純 prompt 壓不下模型的隨機性**：三個邊界案例（大家好／這個是／等一下）在兩次全跑之間
      輪流掉、每次掉的還不一樣，temperature 已經是 0。往 prompt 疊字沒有用，
      **改成給機械式閘門才收斂**：JSON 第一個欄位 `lastLineHasQuestion`（只看最後一則的是非題），
      配一條凌駕全部的硬性規則「沒問題且沒喊名字 → 直接 none」。
      教訓：模型執行「照規則檢查」比「憑判斷」穩定得多，判準寫不穩就把它降維成查表。
  - 驗證腳本（含「不用多帳號開會議」的三種測法分工）：`D:\grp\蜜塔-壓力測試腳本-0818.md`。
  - ⚠️ 副作用要盯著：`TURN_DECISION_SYSTEM` 從 ~2,900 字元長到 ~4,485 字元。
    prompt 一長，舊判準被稀釋的風險就上升，**這次一定要全跑不能只跑 `--only`**。
...
- ⚠️ **環境落差會讓一整套測試失去意義**（2026-08-18 的方法論教訓，代價是一場有指導老師的會議）：
  兩位組員用 12 段劇本把功能逐項測過、結論「大部分沒問題」，但他們的環境**沒裝 cloudflared**，
  逐字稿要 20–30 秒才進得來。所有時間相關的門檻——`INTERJECTION_TURN_SILENCE_MS`（2.5 秒）、
  `ICEBREAKER_SILENCE_MS`（40 秒）、`INTERJECTION_COOLDOWN_MS`（90 秒）、`FOLLOWUP_WINDOW_MS`（90 秒）
  ——在那條慢路徑上根本壓不到。會議當天才把 cloudflared 裝上直接上場，迴圈快了一個數量級，
  這些門檻第一次同時生效，於是搶話、亂接、瞎掰全部一起出現。
  - 教訓一：**驗收環境必須與展示環境逐項對齊**，「功能有沒有做出來」與「在目標時序下會不會壞」是兩回事。
    這與前面「評測與線上呼叫參數要逐項對齊」是同一個病，只是這次發生在部署層。
  - 教訓二：**時序敏感的東西不能只用功能劇本驗**。離線評測（`eval-*.ts`）看的是「判斷對不對」，
    看不到「訊號什麼時候到」。要驗時序得用 `simulate-meeting.ts`，或至少在**與展示相同的環境**跑一次真會議。
  - 教訓三（給我自己）：`FOLLOWUP_WINDOW_MS` 這種「只是成本閘門、判準交給語意層」的設計，
    在**語意層本身被大量觸發**的會議裡等於沒有閘門。設計時要問一句「這個閘門的最壞情況是什麼」。
