...
/**
 * 向 Dify Chatflow 提問（**現役路徑**，`response_mode: 'blocking'`）。
 *
 * 為什麼維持 blocking：實測（2026-07-29，24 筆樣本）difyMs 中位數 10.3 秒、
 * 最大 14.5 秒，穩定且無逾時；而且約 76% 的問題根本不走 Dify（被分類成
 * chitchat / context）。加上 partial ack 1.5 秒內就先應聲，體感不卡 ——
 * 換 streaming 對延遲沒有實質幫助，故先不冒行為改變的風險。
 *
 * 要改用串流時：把下面 resolveAnswer 的呼叫改成 {@link askQuestionStreaming}
 * 即可，兩者回傳契約完全相同。
 */
