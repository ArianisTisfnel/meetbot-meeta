  ...
  try {
    ...
    const composed = await completeText({
      system: [
        ...
        // 與 answerFromTranscript 同一道禁區：合成這一步同樣會憑空長出實作細節。
        '**只能用上面兩個來源的內容**，不可自行補充你自己（蜜塔）的架構、模組、參數或門檻——',
        '兩個來源都沒提到就說沒提到，不要為了把答案講得完整而發明技術名詞或數值。',
      ].join('\n'),
      prompt: `資料查詢結果：\n${factAnswer}\n\n會議近期對話：\n${context}\n\n請回答：${question}`,
      maxTokens: 512,
    })
  ...
  try {
    const text = await completeText({
      system:
        ...
        // 「你怎麼一直插話」「你還在嗎」這類問自身行為的話會分到 chitchat（見 TURN_DECISION ③），
        // 沒有這句就會順口編出模組與門檻（2026-08-18 真會議實測）。
        '\n被問到自己為什麼這樣反應、內部怎麼運作時，坦白說不清楚、請他問開發的同學，' +
        '**不要猜測或發明任何架構、模組、參數、門檻**。',
      prompt: question,
      maxTokens: 100,
    })
...
async function answerFromTranscript(
  ...
  const text = await completeText({
    system: [
      ...
      '',
      // 2026-08-18：真會議中被問「你為什麼會插話／你的語意層是什麼」，蜜塔當著指導老師的面
      // 編出「自動補位邏輯過於激進」「VAD 門檻」「事件驅動流程圖」等本專案根本沒有的東西。
      // 成因就是上面第 2 點——它要求意見型「不要推託說無法回答」，等於授權她硬掰。
      // 這裡開一個凌駕它的禁區，但只圈住「蜜塔自身實作」，不動意見型本來的能力。
      '⚠️ 唯一的硬性禁區：**你自己（蜜塔）的實作細節**。',
      '被問到你為什麼這樣反應、你的架構／模組／參數／門檻／判斷邏輯是什麼時：',
      '- 只能複述逐字稿裡真的有人講過的內容；逐字稿沒講到就直說「這部分我看不到，要問開發的同學」。',
      '- **不可以發明技術名詞或數值**（門檻、權重、緩衝區、流程圖、模組名稱都算）。',
      '  聽起來很專業的猜測比一句「我不知道」傷害大得多——現場會有人把它當成真的實作去改。',
      '- 這條凌駕上面第 2 點：對「你的實作是什麼」這類問題，講不出根據就是要推託。',
      '',
