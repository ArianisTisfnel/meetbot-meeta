...
# demo 專用堆疊：project 名 meetbot-demo、port 5434/9002/9003/5051，與主 repo 完全隔離。
Write-Host "Starting local infrastructure (Postgres, MinIO) [demo stack]..." -ForegroundColor Cyan
docker compose up -d
if ($LASTEXITCODE -ne 0) {
...
$health = $null
do {
    $health = docker inspect meetbot-demo-postgres --format '{{.State.Health.Status}}' 2>$null
...
# 3.5 Bootstrap 身分層資料表（public.users / public.api_tokens）
# 這一版的身分層在 Vexa 的 public schema，Prisma 不管它（見 CLAUDE.md）；原本由
# vexa-init-db 容器建立。demo 用獨立資料庫沒有那個容器，表不存在 → 登入時
# /internal/token 會 500 → 專案清單 401 →「專案清單載入失敗」。這段補上，冪等。
Write-Host "Bootstrapping identity tables (public.users / api_tokens)..." -ForegroundColor Cyan
$sqlFile = "$rootDir\backend\scripts\sql\00-bootstrap-public-schema.sql"
if (Test-Path $sqlFile) {
    # 用 docker cp + psql -f 而不是 PowerShell 管線：管線會依 console 編碼轉碼，
    # SQL 檔裡的中文註解可能被打壞導致語法錯誤；docker cp 是逐位元組複製。
    docker cp $sqlFile meetbot-demo-postgres:/tmp/bootstrap.sql 2>&1 | Out-Null
    docker exec meetbot-demo-postgres psql -U meetbot -d meetbot -v ON_ERROR_STOP=1 -f /tmp/bootstrap.sql 2>&1 | Out-Null
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Warning: identity table bootstrap failed - login will not work." -ForegroundColor Yellow
        Write-Host "  手動重跑看錯誤訊息：" -ForegroundColor Gray
        Write-Host "  docker exec -it meetbot-demo-postgres psql -U meetbot -d meetbot -f /tmp/bootstrap.sql" -ForegroundColor Gray
    } else {
        Write-Host "Identity tables ready." -ForegroundColor Green
    }
} else {
    Write-Host "Warning: $sqlFile not found - skipping identity bootstrap." -ForegroundColor Yellow
}

# 4. Apply Prisma schema (db push) and generate Prisma client
Write-Host "Applying Prisma schema (db push)..." -ForegroundColor Cyan
Set-Location "$rootDir\backend"
# --accept-data-loss：demo 用獨立資料庫（compose project meetbot-demo），沒有要保的資料。
# 不加這個旗標時，schema 與現有資料庫有破壞性差異 → prisma 會停在互動式確認 → start.bat 卡住。
